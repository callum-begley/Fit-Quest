...
function ActivityLog() {
  ...
  if (loading) {
    return <p>Loading your Quest log...</p>
  }
  ...
  return (
    <>
      <div className="mx-auto flex max-w-2xl flex-row items-center justify-center gap-10 overflow-hidden rounded font-mono text-xl text-white">
        <button
          className={`z-30 translate-y-2 rounded-t-lg bg-gray-900 p-4 ring-2 ring-gray-700 ${showLog !== 'challenge' ? 'bg-gray-800 bg-opacity-70' : ''}`}
          onClick={() => setShowLog('challenge')}
        >
          Quest Log
        </button>
        <button
          className={`translate-y-2 rounded-t-lg bg-gray-900 p-4 ring-2 ring-gray-700 ${showLog !== 'sidequest' ? 'bg-gray-800 bg-opacity-70' : ''}`}
      ...
      {showLog === 'challenge' ? (
        <div className="z-20 flex flex-col items-center justify-center pb-4 font-mono text-green-300 ">
